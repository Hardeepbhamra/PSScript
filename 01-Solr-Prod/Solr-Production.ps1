...
#Change the current directory CERT to import cert.
...
if($solrSSL -eq $true)
{
    ...
    if(!(Test-Path -Path "$solrRoot\server\etc\$pfxFile"))
    {
...
# Adding Host Entry to server for Solr Host
$HostFile = 'C:\Windows\System32\drivers\etc\hosts'
#Hosts to add
$Hosts = @("$solrHost")
#Get Contents of the host file
$File = Get-Content $HostFile

#Write the entry to host file, if it does not exist.
foreach ($hostFileEntry in $Hosts)
{
    Write-Host "Checking existing HOST File entry for $hostFileEntry..."
